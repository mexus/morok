...
for fixture in weak_int_add weak_float_neg_zero invalid_where scalar_stack scalar_load gated_load; do
...
expect_equal = sys.argv[1].rsplit('/', 1)[-1].split('-rust.json')[0] in {"weak_int_add", "weak_float_neg_zero", "invalid_where", "scalar_stack"}
