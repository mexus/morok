...
```text
propagate_invalid          -- पहले होना ज़रूरी (x*0=0 से पहले)
...
pm_move_where_on_load       -- masked loads के लिए WHERE->INDEX embedding
```
...
```text
Without ordering:  MUL(0, WHERE(cond, x, Invalid)) -> 0    (x*0=0 fires, Invalid खो जाता है)
With ordering:     MUL(0, WHERE(cond, x, Invalid))
                 -> WHERE(cond, MUL(0, x), Invalid)         (Invalid propagation पहले fire)
                 -> WHERE(cond, 0, Invalid)                  (फिर x*0=0 safe है)
```
