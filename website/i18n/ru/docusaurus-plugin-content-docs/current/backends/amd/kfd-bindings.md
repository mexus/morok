...
- `device/build.rs` запускает `bindgen` **безусловно на каждом Unix-хосте** —
  нет ни отдельной ветки для Linux, ни заглушки-пустышки. Он **герметичен**: ему не
  нужны системные заголовки ядра. Два заголовка, которые `kfd_ioctl.h`
  ...
  одинакового размера) — все 60 структур и 35 констант идентичны. (bindgen'у
...
фичей времени компиляции: каждый `cargo check` на Unix проверяет типы в местах
вызова KFD, а хост без GPU просто никогда не регистрирует фабрику.
...
установлена SDMA-очередь копирования (по умолчанию — см. [Обзор](./overview.md)),
...
`synchronize()`. Обобщённый `LruAllocator` (`device/src/allocator.rs`)
складывает освобождённые буферы в пул по `(size, BufferSpec)`; спецификация
`nolru` обходит пул для code object, scratch и инфраструктуры очередей.
