...
- **Один рендерер.** Тот же путь `svod-codegen`, что опускает графовые ядра в LLVM IR и
  AMD-бинарник, рендерит и ваше `tk`-ядро. Нет второго бэкенда, который пришлось бы писать,
