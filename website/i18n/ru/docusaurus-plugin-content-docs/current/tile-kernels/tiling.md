...
Регистровые тайлы добавляют поля `stride`/`interleave` (`RTBaseShape`), чтобы закодировать
чётную/нечётную карту строк аккумулятора RDNA, которую простым stride не выразить.
...
в `tk/src/arch.rs` разрешает её в верную физическую форму под целевую платформу (CDNA или RDNA).
Эта косвенность и делает одно ядро переносимым между размерами волн; см.
