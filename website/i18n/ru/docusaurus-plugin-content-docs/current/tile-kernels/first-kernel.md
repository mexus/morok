...
| **Это GPU-ядро** | Билдер создаёт настоящие индексы лейнов (`Op::Special`), поэтому исполнение идёт на AMD-устройстве, а не на CPU. |
...
только на AMD LLVM-бэкенде: на скалярном CPU-пути индекс лейна смысла не имеет. А вот *построение*
