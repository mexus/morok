...
关于这条路径多说一句：直接路线会运行一次普通流水线本会推迟到后面才做的重写（把 `Index` 算术降级到目标平台的整数 dtype），因为它有意跳过了平时干这活的那个优化器阶段。于是你不靠调度器也拿到了正确的代码。
...
指纹一旦挪位，就说明你改了所发射的 IR（无论有意无意），金标准测试会逼你正视它。`tk/src/test/unit/golden` 下的单元测试正是用这一招锁住了 matmul 和 Flash Attention 的图。
...
| 「是*设备/驱动层*在捣乱吗？」 | [AMD 后端 → 调试](../backends/amd/debugging) |

最后一行很重要：本章讲的是调试*内核*，也就是你编写的 IR 和它产出的数字。当问题落在那一层之下时（队列派发、内存故障、驱动），[AMD 后端 → 调试](../backends/amd/debugging) 那一章才是该去的地方。
