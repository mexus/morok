//! Affine layer normalization with an optional bias. ModernBERT norms use
//! `bias = false` (gamma-only); the bias slot stays `Option` so the struct
//! generalizes to the tied-decoder / classifier norms if added later.
//!
...
impl HasStateDict for LayerNormWeights {
    ...
    fn load_state_dict(&mut self, sd: &StateDict, prefix: &str) -> std::result::Result<(), state::Error> {
        ...
        // Bias is optional: absent keys (ModernBERT's `norm_bias=false`) leave
        // `bias` as `None`.
