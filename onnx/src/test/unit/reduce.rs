...
morok_tensor::codegen_tests! {
    fn test_registry_reduce_sum(config) {
    ...
    fn test_registry_reduce_max(config) {
        let registry = OpRegistry::new();
    ...
    fn test_registry_reduce_with_keepdims(config) {
        let registry = OpRegistry::new();
        let x = Tensor::from_ndarray(&array![[1.0f32, 2.0], [3.0, 4.0]]);
    ...
    fn test_argmax_select_last(config) {
        let registry = OpRegistry::new();
    ...
    fn test_argmin_select_last(config) {
        let registry = OpRegistry::new();
