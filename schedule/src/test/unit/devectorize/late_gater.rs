...
fn test_move_load_gate_from_index() {
    ...
    let Op::Load { index: result_index, alt: Some(alt), gate: Some(result_gate) } = result.op() else {
    ...
    assert!(matches!(result_index.op(), Op::Index { indices, .. } if Arc::ptr_eq(&indices[0], &index)));
}
...
fn test_move_store_gate_from_index() {
    ...
    let Op::Store { index: result_index, gate: Some(result_gate), .. } = result.op() else {
...
fn test_where_after_gated_load_becomes_load_alt() {
    ...
    let Op::Load { alt: Some(result_alt), gate: Some(result_gate), .. } = result.op() else {
...
fn test_invalid_memory_fold_is_not_overbroad() {
    ...
    let mixed_index = UOp::new(
        Op::Index { buffer: buffer.clone().broadcast(2), indices: vec![mixed_lanes].into() },
    ...
    assert!(
        matches!(result.op(), Op::Store { gate: Some(_), .. }),
...
fn test_final_decomposition_removes_vectorized_data_invalid() {
    ...
    let store = UOp::new(Op::Store { index, value, gate: None }, DType::Void);
...
fn test_final_rewrite_does_not_reintroduce_invalid_memory_index() {
    ...
    let Op::Load { index, alt: Some(_), gate: Some(result_gate) } = result.op() else {
        panic!("final rewrite must preserve the gated LOAD");
    };
    assert!(Arc::ptr_eq(result_gate, &gate));
    assert!(matches!(index.op(), Op::Index { indices, .. } if Arc::ptr_eq(&indices[0], &clean_index)));
}
