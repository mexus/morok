...
fn stacked_index_becomes_stack_of_scalar_indices() {
    ...
    let Op::Stack { sources } = result.op() else { panic!("expected STACK: {}", result.tree()) };
    assert_eq!(sources.len(), 2);
    assert!(sources.iter().all(|source| source.dtype() == DType::Float32));
    assert!(sources.iter().all(|source| matches!(source.op(), Op::Index { indices, .. } if indices.len() == 1)));
}
...
fn reshaped_index_is_fully_consumed_by_shaped_indexing() {
    ...
    assert!(matches!(result.op(), Op::Index { indices, .. } if indices.len() == 1));
...
fn scalar_expand_becomes_stack() {
    ...
    let expanded = UOp::new(
        Op::Expand { src: scalar, new_shape: svod_ir::shape::shape_to_uop(&smallvec::smallvec![4usize.into()]) },
        DType::Float32,
    );
    let result = rewrite(expanded);

    assert!(matches!(result.op(), Op::Stack { sources } if sources.len() == 4));
...
fn void_reshape_is_removed() {
    ...
    let reshape = UOp::new(
        Op::Reshape { src: store.clone(), new_shape: svod_ir::shape::shape_to_uop(&smallvec![]) },
        DType::Void,
    );
...
fn scalar_index_into_stack_folds_without_reconstructing_storage() {
    ...
    let rebuilt = UOp::stack(smallvec![
        UOp::new(Op::Index { buffer: values.clone(), indices: smallvec![UOp::index_const(0)] }, DType::Float32),
        UOp::new(Op::Index { buffer: values.clone(), indices: smallvec![UOp::index_const(1)] }, DType::Float32),
    ]);
    let rebuilt = rewrite(rebuilt);
    assert!(matches!(rebuilt.op(), Op::Stack { .. }));

    let selected = UOp::new(Op::Index { buffer: values, indices: smallvec![UOp::index_const(1)] }, DType::Float32);
...
fn adjacent_movement_ops_are_cleaned_in_upstream_order() {
    ...
    let permuted = UOp::new(
        Op::Permute {
            src: UOp::new(Op::Permute { src: values.clone(), axes: vec![0] }, DType::Float32),
            axes: vec![0],
        },
        DType::Float32,
    );
...
fn child_singleton_reshape_is_visible_to_parent_expand() {
    ...
    let result = rewrite(expanded);
    let Op::Stack { sources } = result.op() else { panic!("expected outer STACK: {}", result.tree()) };
...
fn child_stack_broadcast_is_visible_to_mixed_alu_parent() {
    ...
    assert!(
        matches!(result.op(), Op::Stack { sources } if sources.len() == 4),
