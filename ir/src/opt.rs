//! Kernel optimization directives.
//!
//! These pure-data types describe a single optimization to apply to a kernel
//! (UPCAST/LOCAL/UNROLL/...). They live in `svod_ir` — rather than the
//! `schedule` crate that consumes them — so that [`crate::KernelInfo`] can carry
//! an author-supplied `opts_to_apply` list (the analog of tinygrad's
//! `KernelInfo.opts_to_apply`) without `ir` depending on `schedule`.
//!
//! The `schedule` crate re-exports these (`svod_schedule::optimizer::{Opt,
//! OptOps, OptArg}`) and provides the `OptError`-returning accessor methods via
//! an extension trait, keeping its error type out of `ir`.
