...
fn test_sqrt_positive() {
    assert_eq!(UOp::native_const(9.0f32).try_sqrt().unwrap().dtype(), svod_dtype::DType::Float32);
}

#[test]
fn test_sqrt_zero() {
    // sqrt(0) = 0
    assert_eq!(UOp::native_const(0.0f32).try_sqrt().unwrap().dtype(), svod_dtype::DType::Float32);
}

#[test]
fn test_sqrt_one() {
    // sqrt(1) = 1
    assert_eq!(UOp::native_const(1.0f32).try_sqrt().unwrap().dtype(), svod_dtype::DType::Float32);
}

#[test]
fn test_sqrt_preserves_dtype() {
    assert_eq!(UOp::native_const(4.0f32).try_sqrt().unwrap().dtype(), svod_dtype::DType::Float32);
    assert_eq!(UOp::native_const(4.0f64).try_sqrt().unwrap().dtype(), svod_dtype::DType::Float64);
}
...
fn test_exp2_zero() {
    // 2^0 = 1
    assert_eq!(UOp::native_const(0.0f32).try_exp2().unwrap().dtype(), svod_dtype::DType::Float32);
}

#[test]
fn test_exp2_positive() {
    // 2^3 = 8
    assert_eq!(UOp::native_const(3.0f32).try_exp2().unwrap().dtype(), svod_dtype::DType::Float32);
}

#[test]
fn test_exp2_negative() {
    // 2^-2 = 0.25
    assert_eq!(UOp::native_const(-2.0f32).try_exp2().unwrap().dtype(), svod_dtype::DType::Float32);
}

#[test]
fn test_exp2_one() {
    // 2^1 = 2
    assert_eq!(UOp::native_const(1.0f32).try_exp2().unwrap().dtype(), svod_dtype::DType::Float32);
}
...
fn test_log2_one() {
    // log2(1) = 0
    assert_eq!(UOp::native_const(1.0f32).try_log2().unwrap().dtype(), svod_dtype::DType::Float32);
}

#[test]
fn test_log2_power_of_two() {
    // log2(8) = 3
    assert_eq!(UOp::native_const(8.0f32).try_log2().unwrap().dtype(), svod_dtype::DType::Float32);
}

#[test]
fn test_log2_two() {
    // log2(2) = 1
    assert_eq!(UOp::native_const(2.0f32).try_log2().unwrap().dtype(), svod_dtype::DType::Float32);
}

#[test]
fn test_log2_fractional() {
    // log2(0.5) = -1
    assert_eq!(UOp::native_const(0.5f32).try_log2().unwrap().dtype(), svod_dtype::DType::Float32);
}
...
fn test_sin_zero() {
    ...
    // sin(0) = 0
    assert_eq!(result.dtype(), svod_dtype::DType::Float32);
}
...
fn test_sin_preserves_dtype() {
    ...
    assert_eq!(result_f32.dtype(), svod_dtype::DType::Float32);

    let result_f64 = UOp::native_const(0.5f64).try_sin().unwrap();
    assert_eq!(result_f64.dtype(), svod_dtype::DType::Float64);
}
...
fn test_trunc_near_zero() {
    ...
    // trunc(0.9) = 0.0
    assert_eq!(result_pos.dtype(), svod_dtype::DType::Float32);

    let result_neg = UOp::trunc(UOp::native_const(-0.9f32));
    // trunc(-0.9) = -0.0 (towards zero)
    assert_eq!(result_neg.dtype(), svod_dtype::DType::Float32);
}
...
fn test_reciprocal_twice() {
    ...
    assert_eq!(recip2.dtype(), svod_dtype::DType::Float32);
}
