...
fn format_node(uop: &Arc<UOp>) -> String {
    let op_str = match uop.op() {
        ...
        Op::Param { arg, .. } => format!("PARAM(slot={})", arg.slot),
        Op::Buffer { arg, .. } => format!("BUFFER(slot={}, addrspace={:?})", arg.slot, arg.addrspace),
        Op::Stage { opts, .. } => match &opts.local_axis {
        ...
        Op::GetAddr { device, .. } => format!("GETADDR({})", device.canonicalize()),
        ...
        Op::Reduce { reduce_op, ranges, num_axes, .. } => {
        ...
        Op::AllReduce { reduce_op, device, .. } => format!("ALL_REDUCE({reduce_op:?}, {device:?})"),
        ...
        Op::Ins { arg, .. } => format!("INS({})", arg.opcode),
        ...
        Op::Copy { device, .. } => format!("COPY({device:?})"),
        ...
        Op::Slice { size, .. } => format!("SLICE(size={size})"),
