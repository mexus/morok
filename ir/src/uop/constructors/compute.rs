...
impl UOp {
    ...
    pub fn try_where(condition: Arc<Self>, true_val: Arc<Self>, false_val: Arc<Self>) -> Result<Arc<Self>> {
        ...
        let op = Op::Ternary(TernaryOp::Where, condition, true_val, false_val);
        let dtype = crate::dtype_from_op(&op).expect("WHERE has a bool condition and promotable branches");
        Ok(Self::new(op, dtype))
    }
